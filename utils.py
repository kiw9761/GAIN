...
def reverse_encoding(imputed_data, feature_name, encoder, onehot, ori_data_dim):
  ...
  '''
    encoded_data = imputed_data[:,:(imputed_data.shape[1] - (ori_data_dim - onehot))]
    inversed_data = encoder.inverse_transform(encoded_data)
    imputed_data = np.concatenate((inversed_data, imputed_data[:,(imputed_data.shape[1] - (ori_data_dim - onehot)):]),axis=1)
    return imputed_data